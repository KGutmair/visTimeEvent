...
km_grouped_weighted <- function(data,
  ...
  event_sym <- sym(event)
  data1 <- data %>%
    rename(group1 = !! group_sym,
           time1 = !! time_sym,
           event1 = !! event_sym)



  adjsurv <- adjustedsurv(data=data1,
                          variable="group1",
                          ev_time="time1",
                          event="event1",
                          method="iptw_km",
  ...
  median_surv <- median_surv %>%
    mutate(surv_time = format(round(q_surv, 2), nsmall = 2),
           ci_lower = format(round(ci_lower, 2), nsmall = 2),
           ci_upper = format(round(ci_upper, 2), nsmall = 2),
           CI_95 = paste0(ci_lower, " - ", ci_upper),
           p_value = ifelse(p_value >= 0.1, format(round(p_value, 2), nsmall = 2), format(round(p_value, 3), nsmall = 3)),
           p_value = ifelse(p_value < 0.001, "< 0.001", p_value)) %>%
    select(group, surv_time, CI_95, p_value)
  ...
  surv_probability <- time_table %>%
    group_by(group) %>%
    filter(time < time_survival) %>%
    filter(time == max(time))

  surv_probability <- surv_probability %>%
    mutate(surv = round(surv*100, 0),
           ci_lower = round(ci_lower*100, 0),
           ci_upper = round(ci_upper*100, 0)) %>%
    mutate(surv_prob = paste0( surv, "% (", ci_lower, " - ", ci_upper, ")")) %>%
    select(- c(se, ci_lower, ci_upper)) %>%
    relocate(group, time, surv)
  ...
  if (is.logical(color_curves)) {
    ...
    color_curves <- colorRamps::primary.colors(n + 1)[-1]
  }

  km_plot <- plot(adjsurv, xlab = x_title, ylab = y_title, title = title,
  ...
  if(show_label == "probability") {
    ...
    for (i in seq_len(nrow(median_surv))) {
      label_vec[i] <- paste(median_surv$group[i] , " median  = ", median_table$median_CI[i], sep = " ")
    }
    km_plot <- km_plot +
      scale_color_manual(
        values = color_curves,
  ...
  if(risk_table) {
    risk_tab <- plot_risk_table.groups(data = data,
                                       breaks_x = breaks_x,
                                       x_lim = x_lim,
                                       variable = group,
                                       ev_time = time,
    ...
    km_plot <- cowplot::plot_grid(km_plot, risk_tab, ncol=1, align="v",
...
plot_risk_table.groups <- function(data,
  ...
  plotdata <- get_risk_table.groups(times = breaks_x, data = data,
                                    variable = variable,
                                    ev_time = ev_time,
                                    weights = data[[weight_col]],
                                    event = event,
  ...
  p <- ggplot2::ggplot(data=plotdata, mapping) +
    ...
    ggplot2::scale_y_continuous(breaks=c(0, 0.3, 1, 1.3), labels = c("Event", "At risk",
                                                                     "Event", "At risk")) +
...
#'
#' @inheritParams plot_risk_table.groups
...
get_risk_table.groups <- function(times, data, variable, ev_time, event=NULL,
  ...
  out <- out %>%
    mutate(group = (as.numeric(group) - 1),
           group = ifelse(measure == "n_event", group + 0.3, group),
           group1 = factor(group, levels = c(0, 0.3, 1, 1.3)))
...
get_risk_table.all <- function(times, data, ev_time, event=NULL,
  ...
  n_risk <- vapply(times, function(x){sum(weights[data[, ev_time]>=x])},
                   FUN.VALUE=numeric(1))

  n_event <- vapply(times, FUN=function(x){sum(weights[data[, ev_time] < x &
                                                         data[, event]==1])},
                    FUN.VALUE=numeric(1))
